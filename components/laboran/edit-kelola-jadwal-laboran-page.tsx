...
interface EditKelolaJadwalLaboranPageProps {
  dataClass: TGetClass;
}
...
const Input = ({
  ...
  <input
    ...
    disabled={disabled}
    className={`px-4 py-2 ${theme.highlight_focus} rounded-md transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed ${className}`}
  />
);
...
const Select = ({
  ...
  <select
    ...
    onChange={(e) => onChange(e.target.value)}
    disabled={disabled}
    className={`px-4 py-2 rounded-lg ${theme.highlight_focus} transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed`} //[&>option]:bg-gray-800
  >
...
const EditKelolaJadwalLaboranPage = ({
  dataClass,
}: EditKelolaJadwalLaboranPageProps) => {
...
export default EditKelolaJadwalLaboranPage;
