...
interface DetailKelolaJadwalPageProps {
  dataClass: TGetClass;
}
...
const DetailKelolaJadwalPage = ({ dataClass }: DetailKelolaJadwalPageProps) => {
  if (!dataClass) {
    return (
    ...
    <div className="min-h-screen p-6 lg:ml-48">
      ...
      <div className={`${theme.card_max_4_fit}`}>
        ...
        <div className={`${theme.card_default}`}>
          ...
          <div className={`flex flex-wrap items-center gap-2 text-sm ${theme.text_default_light}`}>
        ...
        <div className={`${theme.card_default}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <InfoCard icon={Building} title="ID Kelas" value={dataClass?.id || "-"} />
            <InfoCard icon={Building} title="Program Studi" value={dataClass?.course?.semester.prodi.name || "-"} />
            <InfoCard icon={Calendar} title="Semester" value={dataClass?.course?.semester.semesterNumber?.toString() || "-"} />
            <InfoCard
              icon={BookOpen}
              title="Mata Kuliah"
        ...
        <div className={`${theme.card_default}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <InfoCard
              icon={Building}
              title="Jam"
              value={`${dataClass?.jadwalPraktikum?.mulai || "-"} - ${dataClass?.jadwalPraktikum?.selesai || "-"}` || "-"}
            />
            <InfoCard icon={Building} title="Hari" value={dataClass?.jadwalPraktikum?.hari || "-"} />
            <InfoCard icon={Calendar} title="Ruangan" value={dataClass?.jadwalPraktikum?.ruangan || "-"} />
          </div>
        </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="space-y-1">
                        <div className={`flex items-center gap-2 text-xs ${theme.text_default}`}>
                          <span>NPM:</span>
                          <span className="font-medium">{dataAsdos.asdos.npm || "-"}</span>
                        </div>
                        <div className={`flex items-center gap-2 text-xs ${theme.text_default}`}>
                          <span>Absensi:</span>
                          <span>{dataAsdos.absensi.length}/16</span>
                        </div>
        ...
        <div className={`${theme.card_default}`}>
          <SectionHeader
            ...
            subtitle={dataClass.classDosen ? `${dataClass.classDosen.length} dosen terdaftar` : "Belum ada dosen terdaftar"}
          />
          ...
          {dataClass.classDosen && dataClass.classDosen.length > 0 ? (
                ...
                <div key={index} className={`${theme.card_sub_background}`}>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`flex items-center gap-2 text-xs ${theme.text_default}`}>
                        <span>NIP:</span>
                        <span className="font-medium">{dataDosen.dosen.nip || "-"}</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="text-center py-8">
              <div className={`${theme.card_sub_background} inline-block`}>
                <User className={`h-8 w-8 ${theme.text_default_light} mx-auto mb-2`} />
                <p className={`${theme.text_default} text-sm`}>Data dosen tidak ditemukan</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className={`${theme.card_default}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <InfoCard icon={Calendar} title="Tanggal Dibuat" value={formatDate(dataClass?.createdAt ?? new Date())} />
            <InfoCard icon={Calendar} title="Terakhir Diperbarui" value={formatDate(dataClass?.updatedAt ?? dataClass?.createdAt ?? new Date())} />
          </div>
        </div>
      </div>
    </div>
...
export default DetailKelolaJadwalPage;
