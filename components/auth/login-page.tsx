...
import { useRouter, useSearchParams } from "next/navigation"; // 💡 1. Tambahkan import useSearchParams

// Komponen internal agar useSearchParams aman dari pembongkaran build Next.js (bila menggunakan Static Generation)
const LoginFormInner = () => {
  const router = useRouter();
  const searchParams = useSearchParams(); // 💡 2. Inisialisasi useSearchParams
  const callbackUrl = searchParams.get("callbackUrl"); // 💡 3. Ambil parameter callbackUrl jika ada
  ...
  const onSubmit = async (data: TSignInSchema) => {
    ...
    try {
      // 💡 4. Kirim data kredensial beserta callbackUrl ke server action
      ...
      if (result?.success) {
        ...
        // 💡 5. Alihkan rute secara dinamis berdasarkan respons dari server action
    ...
    <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-2xl p-8 shadow-[0_8px_32px_0_rgba(11,94,168,0.08)]">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <button
          ...
          className="w-full mt-2 bg-[#0B5EA8] hover:bg-[#094d8a] text-white py-3 px-6 font-bold rounded-xl shadow-md shadow-blue-500/10 hover:shadow-lg hover:shadow-blue-500/20 disabled:bg-slate-400 disabled:cursor-not-allowed transform hover:scale-[1.01] active:scale-[0.99] transition-all duration-200 text-sm tracking-wide cursor-pointer"
        >
...
const LoginPage = () => {
  return (
    <div
      className={`min-h-screen relative isolate overflow-x-hidden ${theme.root_background} flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 pt-32 pb-20`}
    >
      <style>{`
          .unila-dot-matrix {
      ...
      <div className="max-w-md w-full relative z-10">
        {/* 💡 Membungkus form di dalam Suspense boundary wajib dilakukan di Next.js saat memanggil useSearchParams */}
        <Suspense
          fallback={
            <div className="bg-white/70 backdrop-blur-md border border-white/40 rounded-2xl p-8 text-center text-slate-500 text-sm font-semibold shadow-md">
              Memuat sistem login...
            </div>
          }
        >
        ...
        <div className="text-center mt-6">
          <p className="text-slate-400 text-[11px] font-medium">
            ...
            <Link
              href="/terms"
              className="text-slate-500 hover:text-[#0B5EA8] underline"
            >
              Syarat & Ketentuan
            </Link>{" "}
            dan{" "}
